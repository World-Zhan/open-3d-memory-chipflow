...
RUN = ROOT / 'runs/croc-io-ring-pg-20260905-001'
...
RUN.mkdir()
script = STAGE / 'probe_io_ring_pg.py'
ast.parse(script.read_text())
report_script = ROOT / 'reports/bondpad/probe_io_ring_pg.py'
if report_script.exists():
    raise FileExistsError(report_script)
shutil.copy2(script, report_script)
shutil.copy2(script, RUN / script.name)
shutil.copy2(Path(__file__), RUN / 'run_io_ring_pg.py')
command = ['docker', 'run', '--rm', '--name', RUN.name, '--cpus', '2', '--memory', '4g',
           ...
           IMAGE, '--signal=TERM', '--kill-after=3s', '120s', 'python3', '/output/probe_io_ring_pg.py']
started = now()
with (RUN / 'tool.log').open('w') as log:
    completed = subprocess.run(command, stdout=log, stderr=subprocess.STDOUT)
manifest = {'classification': 'read_only_io_ring_PG_geometry_not_signoff', 'command': command,
            'started_at': started, 'finished_at': now(), 'returncode': completed.returncode,
            'limits': {'cpus': 2, 'memory_gb': 4, 'container_internal_timeout_seconds': 120},
