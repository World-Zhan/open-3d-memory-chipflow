...
text = DEF.read_text()
assert re.search(r'UNITS DISTANCE MICRONS\s+1000\s*;', text)
...
regions = {}
layer_presence = {}
for name, pair in {'Activ': (1, 0), 'EdgeSeal': (39, 0), 'Recog': (99, 0), 'Passiv': (9, 0), 'dfpad': (41, 0),
                   'Metal3': (30, 0), 'Metal4': (50, 0)}.items():
    index = layout.find_layer(*pair)
    layer_presence[name] = index is not None
    raw = k.Region() if index is None else k.Region(sealed.begin_shapes_rec_touching(index, roi))
...
edge_active_no_recog = edge_active - regions['Recog']
...
distances = {'opening_to_active': min_distance(opening, active, dbu),
             ...
             'opening_to_edge_seal_active_not_recog': min_distance(opening, edge_active_no_recog, dbu),
             'opening_to_edge_seal_drawing': min_distance(opening, regions['EdgeSeal'], dbu)}
# Analytic translations only: neither source nor a candidate GDS is written.
official_candidate_opening = k.Region(k.Box(7100, -67900, 72900, -2100)).transformed(k.Trans(0, 17400))
candidate = {'classification': 'analytical_region_translation_not_layout_or_drc',
             'pad_inward_shift_um': 17.4, 'selected_io_inward_shift_um': 21.5, 'seal_shift_um': 0,
             'opening_bbox_um': bbox(official_candidate_opening.bbox(), dbu),
             'to_shifted_selected_io_active': min_distance(official_candidate_opening, io_local.transformed(k.Trans(0, 21500)), dbu),
             'to_unshifted_other_active': min_distance(official_candidate_opening, active - io_local, dbu),
             'to_uniformly_shifted_neighborhood_active': min_distance(official_candidate_opening, active.transformed(k.Trans(0, 21500)), dbu),
             'to_unchanged_edge_active_not_recog': min_distance(official_candidate_opening, edge_active_no_recog, dbu)}
result = {'schema_version': '1.0.0', 'classification': 'read_only_neighborhood_geometry_not_signoff',
          ...
          'regions': samples, 'layer_presence_in_gds': layer_presence, 'distance_brackets_um': distances,
          'recog_overlap_with_edge_active_area_um2': (regions['Recog'] & edge_active).area()*dbu**2,
          'analytical_official_candidate': candidate,
