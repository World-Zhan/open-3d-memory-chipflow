...
def run(root, run_id):
    ...
    files = [source_netlist, source_checkpoint, geometry_path, pg_path,
        root/'scripts/run_croc_io_floorplan.py', root/'scripts/add_io_ring_connected_leads.tcl',
    ...
    command = ['docker', 'run', '--rm', '--name', run_id, '--cpus', '2', '--memory', '4g', '--user', f'{os.getuid()}:{os.getgid()}',
