...
"""Create isolated pad/IO fixtures then execute unchanged complete public DRC."""
...
def run(root,rid):
    if not re.fullmatch(r'croc-bondpad-io-ab-[a-zA-Z0-9_-]+',rid):raise ValueError('Independent croc-bondpad-io-ab-* run ID required')
    out=root/'runs'/rid;out.mkdir(exist_ok=False)
    inputs=out/'inputs';inputs.mkdir()
    deck=root/'upstream/ihp-open-pdk/ihp-sg13g2/libs.tech/klayout/tech/drc'
    files=[root/'scripts'/n for n in ('build_bondpad_io_fixture.py','run_bondpad_io_fixture.py','run_croc_bondpad_drc_ab.py','audit_bondpad_drc.py')]
    ...
    build=common+['--name',rid+'-build','--entrypoint','python3','-v',f'{inputs}:/output:rw',IMAGE,
                  '/work/scripts/build_bondpad_io_fixture.py']
    manifest={'run_id':rid,'classification':'pad_io_fixture_not_chip_signoff','started_at':now(),
    ...
    audit['note']='Complete public DRC on pad/IO fixtures; pinned antenna deck completion checked by 31 categories and terminal Ant.i.'
