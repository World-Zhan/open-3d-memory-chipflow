...
def check(root,source):
    ...
    for row in placements:
        ...
        size=(70000,70000) if row['master']=='bondpad70_m2_ring' else (180000,180000) if row['master']=='sg13g2_Corner' else (10000,180000) if row['master']=='sg13g2_Filler10000' else (80000,180000)
    ...
    return {'classification':'io_ring_stream_geometry_not_signoff','source_sha256':{str(p.relative_to(root)):sha(p) for p in
       (source/'manifest.json',source/'io_ring.gds',source/'io_ring_sealed.gds',source/'lead_boxes.tsv',floor/'instances.tsv',root/'scripts/check_io_ring_geometry.py')},
