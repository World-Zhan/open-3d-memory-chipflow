...
"""Run bounded CTS and hold repair from structurally audited full placement."""
import argparse
import json
import os
import re
...
def run(root, run_id, structural_audit):
    if not re.fullmatch(r'croc-full-io-cts-[a-zA-Z0-9_-]+',run_id):
        raise ValueError('Independent CTS run ID required')
    source=root/'runs/croc-full-io-placement-20260905-001'
    gate_path=root/structural_audit
    gate=json.loads(gate_path.read_text())
    if gate.get('overall_result') != 'PASS' or gate.get('run_id') != source.name:
        raise ValueError('Source placement structural prerequisite failed')
    ...
    copy(source/'work/openroad/save/02_croc.placed.zip',work/'save/02_croc.placed.zip')
    ...
    copy(original/'reports/03_croc.cts.rpt',dest/'inputs/baseline_cts.rpt')
    with zipfile.ZipFile(original/'save/03_croc.cts.zip') as archive:
        (dest/'inputs/baseline_cts.def').write_bytes(archive.read('03_croc.cts.def'))
    sources[str((original/'save/03_croc.cts.zip').relative_to(root))]=sha(original/'save/03_croc.cts.zip')
    for path in [root/'scripts/run_croc_full_cts.py',root/'scripts/run_io_ring_maximal_diagnostic.py',root/'scripts/run_io_ring_floorplan.py',root/'upstream/croc/env.sh']:
        sources[str(path.relative_to(root))]=sha(path)
    ...
    stage=work/'scripts/03_cts.tcl'
    text=stage.read_text()
    text=replace_once(text,'load_checkpoint 02_${proj_name}.placed','load_checkpoint 02_${proj_name}.placed\nset_thread_count 2\nsource /output/capture_placement.tcl\nverify_placement_ports\ncapture_placement before\ncheck_setup > /output/input_constraints.rpt')
    text=replace_once(text,'-sink_clustering_enable','-sink_clustering_enable -sink_clustering_size 8')
    text=replace_once(text,'# Place inserted cells','puts {Repair hold with propagated clocks and original setup limits}\nrepair_timing -hold -verbose\n\n# Place inserted cells')
    text=replace_once(text,'report_metrics "03_${proj_name}.cts"','source /output/after_placement.tcl\nreport_metrics "03_${proj_name}.cts"')
    ...
    command=['docker','run','--name',run_id,'--cpus','2','--memory','8g','--ulimit','core=0',
        ...
        'source /work/upstream/croc/env.sh && timeout --verbose --signal=TERM --kill-after=15s 900s openroad -exit scripts/03_cts.tcl']
    manifest={'schema_version':'1.0.0','run_id':run_id,'source_run':source.name,'started_at':now(),'status':'running',
        'classification':'full_functional_cts_and_hold_repair_candidate_not_route_or_signoff','command':command,'source_sha256':sources,
        'generated_inputs_sha256':{str(p.relative_to(dest)):sha(p) for p in dest.rglob('*') if p.is_file()},
        'original_stage_operations_preserved':True,'cts_sink_clustering_size':8,'hold_repair_added':True,'thread_limit':2,'io_load_constraint_unchanged':True,
    ...
    samples=[]
    outer_expired=False
    print(json.dumps({'run_id':run_id,'status':'starting','input_checkpoint_sha256':sha(work/'save/02_croc.placed.zip')}),flush=True)
    ...
    complete=bool(returncode==0 and state.get('Status')=='exited' and state.get('ExitCode')==0
        ...
        and 'FULL_PLACEMENT_EVIDENCE_CAPTURE_COMPLETE' in log and 'Stage 03 complete:' in log
        and (work/'save/03_croc.cts.zip').is_file())
...
if __name__=='__main__':
    ...
    raise SystemExit(run(Path(__file__).resolve().parents[1],args.run_id,args.structural_audit))
